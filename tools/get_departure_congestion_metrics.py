...
def get_departure_congestion_metrics(airport_icao):
    ...
    delayed_flights = [
        ...
        if flight["dep_delayed"] is not None
        and flight["dep_delayed"] > 0
    ]
    ...
    average_delay = (
        sum(flight["dep_delayed"] for flight in delayed_flights)
        / len(delayed_flights)
        if delayed_flights
        else 0
    )
